...
KLIFS_KINASE_NAMES_URL = "https://klifs.net/api_v2/kinase_names?species=Human"
KLIFS_KINASE_INFO_URL = "https://klifs.net/api_v2/kinase_information"
...
def fetch_kinase_metadata(
    ...
    if cache_path is not None and cache_path.exists():
        ...
        print(f"Fetching kinase IDs from {KLIFS_KINASE_NAMES_URL} ...")
        r = requests.get(KLIFS_KINASE_NAMES_URL, timeout=30)
        r.raise_for_status()
        kinase_ids = [entry["kinase_ID"] for entry in r.json()]

        ids_param = ",".join(str(k) for k in kinase_ids)
        print(f"Fetching kinase info for {len(kinase_ids)} kinases ...")
        r2 = requests.get(KLIFS_KINASE_INFO_URL, params={"kinase_ID": ids_param}, timeout=60)
        r2.raise_for_status()
        df = (
            pd.DataFrame(r2.json())[["uniprot", "gene_name", "family"]]
            .rename(columns={"uniprot": "kinase_id", "gene_name": "kinase_name", "family": "kinase_family"})
